...
def velocity_field(xy: np.ndarray, t: float, params: ModelParams) -> np.ndarray:
    """
    Seitenansicht-Kinematik:
    - Grundschub nach +x
    - nahe der Platte: Zug zur nächstgelegenen Öffnung (slot_pull), damit Material zu Öffnungen findet
    - hinter der Platte: zusätzlicher Transport nach +x
    """
    x = xy[:, 0]
    y = xy[:, 1]
    ...
    # Zug zu Öffnungen nur in der Nähe der Platte (Stau-/Umlenkzone)
    band = np.abs(x - params.plate_x) < params.pull_band
    if np.any(band):
        target = nearest_slot_center(y[band], params)
        vy[band] += params.slot_pull * (target - y[band])  # zieht Richtung Schlitzzentrum

    # Hinter der Platte (Abbaukammer): mehr Transport nach +x
...
def simulate(params: ModelParams):
    ...
    for k, t in enumerate(ts):
        ...
        # Domain clamp (optional)
        xy_new[:, 0] = np.clip(xy_new[:, 0], params.x_min, params.x_max)
